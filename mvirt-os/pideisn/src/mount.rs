...
fn mount_one(mp: &MountPoint) -> Result<()> {
    if is_mounted(mp.target) {
        log_info!("{} already mounted", mp.target);
        return Ok(());
    }
    ...
    log_info!("Mounted {} on {}", mp.fstype, mp.target);
    Ok(())
}
...
pub fn mount_all() {
    for mp in MOUNTS {
        if let Err(e) = mount_one(mp) {
            log_error!("Failed to mount {}: {}", mp.target, e);
        }
    }
...
fn create_device_nodes() {
    ...
    for (path, major, minor) in nodes {
        ...
        if let Err(e) = nix::sys::stat::mknod(
            ...
            log_warn!("Failed to create {}: {}", path, e);
        }
    }
    ...
    if !pts_path.exists() {
        if let Err(e) = fs::create_dir(pts_path) {
            log_warn!("Failed to create /dev/pts: {}", e);
            ...
            log_warn!("Failed to mount /dev/pts: {}", e);
        }
    }
}
