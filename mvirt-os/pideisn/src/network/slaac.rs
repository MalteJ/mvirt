...
use crate::network::interface::mac_to_eui64;
use crate::network::{Interface, NetlinkHandle};
...
pub async fn configure(iface: &Interface, nl: &NetlinkHandle) -> Result<(), NetworkError> {
    ...
    log_info!("SLAAC: Configuring link-local {}", link_local);
    ...
    for attempt in 0..3 {
        ...
        match timeout(Duration::from_secs(2), receive_ra(&socket)).await {
            ...
            Ok(Err(e)) => {
                log_debug!("SLAAC: Error receiving RA: {}", e);
            }
            Err(_) => {
                log_debug!("SLAAC: Timeout waiting for RA");
            }
        }
    }

    // No router found, but link-local is configured - that's okay
    log_info!("SLAAC: No router found, using link-local only");
    Ok(())
}
...
async fn process_router_advertisement(
    ...
    log_info!("SLAAC: Received RA from {}", ra.router_addr);
    ...
    for prefix_info in &ra.prefixes {
        ...
        if let Err(e) = nl
            ...
            .await
        {
            log_debug!("SLAAC: Failed to add address: {}", e);
        }
    }

    // Add default route if router lifetime > 0
    if ra.router_lifetime > 0 {
        log_info!("SLAAC: Adding default route via {}", ra.router_addr);
        if let Err(e) = nl.add_route_v6(ra.router_addr, iface.index).await {
            log_debug!("SLAAC: Failed to add route: {}", e);
        }
    }
