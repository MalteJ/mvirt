...
use crate::network::{DelegatedPrefix, Dhcp6Lease, Interface};
...
impl Dhcp6Client {
    pub fn new(iface: &Interface, request_pd: bool) -> Result<Self, NetworkError> {
    ...
    pub async fn run(&mut self) -> Result<Dhcp6Lease, NetworkError> {
        ...
        loop {
            ...
            // Send SOLICIT
            log_info!("DHCPv6: Sending SOLICIT on {}", self.iface_name);
            ...
            let advertise = match self.wait_for_advertise(xid, Duration::from_secs(4)).await {
                ...
                Err(NetworkError::Timeout) => {
                    ...
                    log_debug!("DHCPv6: Timeout waiting for ADVERTISE, retry {}", retry);
                    continue;
                }
                Err(e) => return Err(e),
            };

            log_info!("DHCPv6: Received ADVERTISE");

            // Send REQUEST
            log_info!("DHCPv6: Sending REQUEST");
            self.send_request(xid, &advertise)?;

            // Wait for REPLY
            match self.wait_for_reply(xid, Duration::from_secs(4)).await {
                Ok(lease) => {
                    log_info!("DHCPv6: Received REPLY");
                    return Ok(lease);
                }
                Err(NetworkError::Timeout) => {
                    retry += 1;
