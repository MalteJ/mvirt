...
use crate::network::{Dhcp4Lease, Interface};
...
impl Dhcp4Client {
    pub fn new(iface: &Interface) -> Result<Self, NetworkError> {
    ...
    pub async fn run(&mut self) -> Result<Dhcp4Lease, NetworkError> {
        const MAX_RETRIES: u32 = 4;
        let mut retry = 0;

        loop {
            // Send DISCOVER
            log_info!("DHCPv4: Sending DISCOVER on {}", self.iface_name);
            self.send_discover()?;
            ...
            let offer = match self.wait_for_offer(Duration::from_secs(4)).await {
                ...
                Err(NetworkError::Timeout) => {
                    ...
                    log_debug!("DHCPv4: Timeout waiting for OFFER, retry {}", retry);
                    continue;
                }
                Err(e) => return Err(e),
            };

            log_info!("DHCPv4: Received OFFER for {}", offer.offered_ip);

            // Send REQUEST
            log_info!("DHCPv4: Sending REQUEST for {}", offer.offered_ip);
            self.send_request(&offer)?;

            // Wait for ACK
            match self.wait_for_ack(Duration::from_secs(4)).await {
                Ok(lease) => {
                    log_info!("DHCPv4: Received ACK, lease time {}s", lease.lease_time);
                    return Ok(lease);
                }
                Err(NetworkError::DhcpNak) => {
                    log_debug!("DHCPv4: Received NAK, restarting");
                    retry = 0;
                    continue;
                }
                Err(NetworkError::Timeout) => {
    ...
    async fn recv_packet(&self) -> Result<Message, NetworkError> {
        // Use blocking recv in a spawn_blocking since socket2 doesn't have async
...
fn generate_xid() -> u32 {
    // Use a simple random based on time and process info
