...
pub fn create_router_solicitation(src_mac: [u8; 6]) -> Vec<u8> {
    ...
    // ICMPv6 RS
    let icmpv6_start = VIRTIO_NET_HDR_SIZE + ETHERNET_HDR_SIZE + IPV6_HDR_SIZE;
...
fn build_dhcpv6_packet(
    ...
    let total_len = VIRTIO_NET_HDR_SIZE + ETHERNET_HDR_SIZE + IPV6_HDR_SIZE + udp_len;
    ...
    let udp_start = VIRTIO_NET_HDR_SIZE + ETHERNET_HDR_SIZE + IPV6_HDR_SIZE;
...
mod tests {
    ...
    fn test_ns_packet_structure() {
        ...
        // Verify packet length: virtio(12) + eth(14) + ipv6(40) + icmpv6(32) = 98
        assert_eq!(packet.len(), 98);

        // Parse Ethernet
        let eth_data = &packet[VIRTIO_NET_HDR_SIZE..];
        let eth = EthernetFrame::new_checked(eth_data).expect("Ethernet parse failed");
        ...
        // Target address at bytes 8-23 (after type, code, checksum, reserved)
